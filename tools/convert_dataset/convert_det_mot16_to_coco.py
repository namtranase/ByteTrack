...
SPLITS = ['test_det']  # --> split training data to train_half and val_half.
HALF_VIDEO = True
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            if split == "test_det":
                print('{} ann images'.format(int(dets[:, 0].max())))
            for i in range(dets.shape[0]):
                frame_id = int(dets[i][0])
                if frame_id - 1 < image_range[0] or frame_id - 1 > image_range[1]:
                    continue
                track_id = int(dets[i][1])
                cat_id = int(dets[i][7])
                ann_cnt += 1
                if not ('15' in DATA_PATH):
                    #if not (float(anns[i][8]) >= 0.25):  # visibility.
                        #continue
                    if not (int(dets[i][6]) == 1):  # whether ignore.
                        continue
                    if int(dets[i][7]) in [3, 4, 5, 6, 9, 10, 11]:  # Non-person
                        continue
                    if int(dets[i][7]) in [2, 7, 8, 12]:  # Ignored person
                        #category_id = -1
                        continue
                    else:
