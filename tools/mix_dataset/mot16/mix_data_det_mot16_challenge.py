...
MOT16 = json.load(open('datasets/MOT16/annotations/train.json','r'))

img_list = list()
for img in MOT16['images']:
    img['file_name'] = 'MOT16_train/' + img['file_name']
    img_list.append(img)

ann_list = list()
for ann in MOT16['annotations']:
    ann_list.append(ann)

video_list = MOT16['videos']
category_list = MOT16['categories']
...
MOT16_test_det_json = json.load(open('datasets/MOT16/annotations/test_det.json','r'))
img_id_count = 0
for img in MOT16_test_det_json['images']:
    img_id_count += 1
    img['file_name'] = 'MOT16_test_det/' + img['file_name']
    img['frame_id'] = img_id_count
    img['prev_image_id'] = img['id'] + max_img
    img['next_image_id'] = img['id'] + max_img
    img['id'] = img['id'] + max_img
...
for ann in MOT16_test_det_json['annotations']:
    ann['id'] = ann['id'] + max_ann
    ann['image_id'] = ann['image_id'] + max_img
    ann_list.append(ann)
...
json.dump(mix_json, open('datasets/mix_det_mot16/annotations/train.json','w'))